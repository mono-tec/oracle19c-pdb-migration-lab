...
REM ===== Oracle 19c DBCA Silent Create (Destination) =====
...
echo [INFO] Destination DBCA silent create started.
...
call "%DBCA_EXE%" -silent -createDatabase -responseFile "%RESPONSE_FILE%" > "%LOG_DIR%\create_dest_stdout.log" 2>&1

if errorlevel 1 (
  echo [ERROR] Destination DBCA silent create failed.
  echo [ERROR] Check batch log: %LOG_DIR%\create_dest_stdout.log
  echo.
  echo [ERROR] Press any key to exit...
  pause >nul
  exit /b 1
...
echo [INFO] Destination DBCA silent create completed.
echo [INFO] Check log: %LOG_DIR%\create_dest_stdout.log
echo.
